...
$pdf_file = $documento.'_'.$fecha2.pdf;
//
//$pdf->Output("$pdf_file.pdf","I");
$pdf->Output("$_SERVER[DOCUMENT_ROOT]docs/pdf/consentimientos/$pdf_file", 'F');
//$pdf->Output("../resultados/$pdf_file.pdf","F");
    
    $updateSQL = sprintf("UPDATE users SET consentimiento = 1, consentimiento_file = %s, ip = %s, fecha_consentimiento = %s WHERE username = %s", GetSQLValueString($pdf_file, "text"), GetSQLValueString(getUserIpAddr(), "text"), GetSQLValueString($fecha2, "date"), GetSQLValueString($documento, "text"));

    mysql_select_db($database_klinica, $booking);
    $Result1 = mysql_query($updateSQL, $booking) or die(mysql_error());

$insertGoTo = '../../booking/index.php';
